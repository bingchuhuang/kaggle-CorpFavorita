...
oil = oil.interpolate().unstack(level=-1).reset_index()
...
oil = oil[df_2017.columns]
    
items = items.reindex(df_2017.index.get_level_values(1))
oil = oil.reindex(df_2017.index.get_level_values(1))
...
def prepare_dataset(t2017, is_train=True):
    X = pd.DataFrame({
        ...
        "oil_prev_7_2017": get_timespan(oil, t2017, 7, 7).mean(axis=1).values,
        "oil_prev_14_2017": get_timespan(oil, t2017, 14, 14).mean(axis=1).values,
        "oil_prev_30_2017": get_timespan(oil, t2017, 30, 30).mean(axis=1).values,
    })

    for i in range(16):
        X["oil_{}".format(i)]: oil[t2017+timedelta(days=i)].values.astype(np.float16)
        X["day_{}".format(i)]: df_2017[t2017-timedelta(days=16-i)].values.astype(np.float16)
        X["holiday_{}".format(i)]: holiday_2017[t2017-timedelta(days=16-i)].values.astype(np.float16)
...
df_val.to_csv('out/lgbm_promo_hol_pred.csv', float_format='%.4f', index=None)
df_true.to_csv('out/lgbm_promo_hol_true.csv', float_format='%.4f', index=None)
...
del item_nbr_u
submission[['id','unit_sales']].to_csv('lgb_promo_hol.csv', float_format='%.4f', index=None)
